...
graph TD
    ...
    T-MIN-021["T-MIN-021<br/>Design the middle-tier court-card study format (spec + one SUIT-CUPS-12 pilot)"]:::done
    ...
    T-MIN-025["T-MIN-025<br/>Author the nine remaining Coins pip cards to the light-tier suit-card format"]:::review
    T-MIN-024["T-MIN-024<br/>Author the ten remaining Cups pip cards to the light-tier suit-card format"]:::done
    ...
    T-MIN-022["T-MIN-022<br/>Author the ten remaining Swords pip cards to the light-tier suit-card format"]:::done
...
- SCOUT-F6 CONFIRMED THE GAP AND ITS EVIDENCE BASE (dry run against branch test @82721eb, 2026-08-13): research/pilots/SUIT_CARD_FORMAT_SPEC.md (T-MIN-013, merged) is the light tier and covers only the 56 suit cards uniformly; its §6 already flags the gap it does not fill ("the comparison doc ... finds the 16 courts carry load-bearing pattern findings (hybrid anatomy, gender presentation) that strain this format; it recommends the human consider a middle tier for courts"). research/pilots/SUIT_TIER_COMPARISON_T-MIN-013.md §4 states the actual finding driving this: "CUPS-12 (court) is tighter than comfortable: the hybrid-anatomy finding, its relatedness qualification, the gender-presentation caution, and the suit-by-suit grid dependency ... are all load-bearing pattern findings that the light format holds only by leaning on an existing full dossier. The other 15 courts have no such dossier to lean on" — and its recommendation: "consider a middle tier for the 16 courts — the same skeleton plus a bounded 'pattern findings' subsection under §4 ... and 100% rather than sampled verification for the four Cavaliers." No middle-tier spec file exists anywhere in research/pilots/ as of this commit — confirmed by find/grep, see verification_command.
- Write research/pilots/COURT_CARD_FORMAT_SPEC.md, a NEW format spec for the 16 court cards (Fantina/Fante, Cavalier, Queen, King × 4 suits) that EXTENDS research/pilots/SUIT_CARD_FORMAT_SPEC.md — inherit its skeleton (§3), grading legend (§2: [F]/[SI]/[SCH]/[LT]/[PS]/[U]/[UNVERIFIED]), claim-namespace rule (§4), and FORBIDDEN list (§5) verbatim or by explicit reference; do not duplicate them wholesale or silently diverge from them. ADD one new section (a bounded subsection under the iconography baseline, matching the comparison doc's §4 recommendation) for documented pattern-variation content: the format must be able to present MULTIPLE authentic historical forms for a card side by side when specimen evidence supports it (the Cavalier of Cups hybrid-bodied-vs-ordinary-rider case, per D2, is the concrete example), with EACH form's evidentiary basis and confidence stated individually — not a single blended description, and not one form silently preferred over the other.
- Scope boundary on the policy question: the new section IMPLEMENTS D2's "show both forms" decision — it must not relitigate, second-guess, or add conditions to whether variant forms get shown. D2 already decided that. The spec's job is the mechanics: how a form is stated, sourced, and confidence-graded, and how multiple forms coexist in one card study without either being flattened into the other or asserted as universal (the light tier's §5 FORBIDDEN item "universalizing one specimen or pattern" carries forward unchanged).
- Write ONE pilot card to the new middle tier: SUIT-CUPS-12 (Cavalier of Cups) — deliberately the SAME card already covered by both the light-tier pilot (research/pilots/drafts/STANDARD_SUIT-CUPS-12_Cavalier_of_Cups.md) and the full dossier (research/pilots/Pilot2_SUIT-CUPS-12_Cavalier_of_Cups.md + its JSON), so there are now three treatments of the identical card for direct comparison. This is intentional redundancy, matching the project's established T-MIN-013 pattern for format-design tasks — do not substitute a different card. The pilot's pattern-findings section must cover the hybrid-anatomy vs ordinary-rider finding: BM 1896,0501.35 (corroborated by .103) catalogs ordinary men on horseback "instead of monsters with human bodies," the Cups Cavalier survives in that pack, and hybrid body is pattern-selectable rather than rank-defining — carried at the same confidence the full dossier gives it (catalog evidence, not re-opened at this tier = [UNVERIFIED], per the light-tier pilot's own precedent at claim CU12-C06), never upgraded.
- Write a short comparison note (e.g. research/pilots/COURT_TIER_COMPARISON_T-MIN-021.md, or a section appended to COURT_TIER_COMPARISON... naming is the worker's call as long as it references all three tiers by filename) answering: what the middle tier adds over the light tier (specifically — does its pattern-findings section capture the hybrid-anatomy finding, its BM .35/.103 relatedness qualification, and the gender-presentation caution as well as the full dossier does, better, or worse), and what the middle tier still does NOT carry versus the full dossier (if anything remains). This is the evidence a human needs to decide whether the middle tier suffices for all 16 courts, or whether some edge case (e.g. a court with MORE than two documented variant forms, or no full dossier to lean on at all) still needs full treatment — flag that open question explicitly if the pilot doesn't resolve it, don't paper over it.
- OUT OF SCOPE (do not do this work): rolling the middle tier out to the other 15 court cards: writing prompts, art-direction guidance, or deck-generation-pipeline material for branch-variation cards (D2's own consequence list in tasks/human/editorial_decisions_2026-08-12.md names this as separate follow-up); and updating the Visual Canon policy document (also D2's own separate follow-up item). This task is format design plus one pilot and its comparison note, nothing more.

**Definition of Done:**
- research/pilots/COURT_CARD_FORMAT_SPEC.md exists, explicitly extends (references, does not silently fork) SUIT_CARD_FORMAT_SPEC.md's skeleton/legend/claim-namespace/FORBIDDEN list, and adds a pattern-variation subsection requiring each documented historical form to carry its own evidentiary basis and confidence grade, per D2's "show both forms" decision (implemented, not relitigated).
- One SUIT-CUPS-12 (Cavalier of Cups) pilot exists written to the new middle tier, distinct from both research/pilots/drafts/STANDARD_SUIT-CUPS-12_Cavalier_of_Cups.md (light) and research/pilots/Pilot2_SUIT-CUPS-12_Cavalier_of_Cups.md (full), and its pattern-findings section states the hybrid-vs-ordinary-rider finding with per-form evidentiary basis and confidence, carried at or below the full dossier's confidence (no upgrading).
- A comparison note exists, references all three tiers (light/middle/full) by filename, and states explicitly what the middle tier adds over light and what (if anything) it still lacks versus full, including whether that gap is acceptable for rollout to the other 15 courts or leaves an open edge case.
- Rollout to the other 15 court cards, prompt/art-direction guidance, and the Visual Canon policy document are NOT touched by this task.
- human_review_required is honored — task ends at HUMAN_REVIEW with the format proposal and one pilot, not with additional court cards authored or the Visual Canon policy updated.

*Audited against SHA:* `82721eb`
...
- COURTS EXCLUDED (confirm, do not re-derive from scratch): SUIT-SWORDS-11 (Jack/Fante), SUIT-SWORDS-12 (Cavalier/Cavallo), SUIT-SWORDS-13 (Queen/Regina), SUIT-SWORDS-14 (King/Re) are the four Swords court cards, sort orders 11-14, already scoped under the middle court tier (T-MIN-013 pilot 2 of 2; T-MIN-021 middle-tier format, closed DONE). They are OUT OF SCOPE here -- do not write, restate, or touch any Swords court file under this task.
- Write all ten pip cards fresh, one file per card, to research/pilots/drafts/STANDARD_SUIT-SWORDS-NN_Card_Name.md (NN zero-padded, e.g. STANDARD_SUIT-SWORDS-01_Ace_of_Swords.md), following research/pilots/SUIT_CARD_FORMAT_SPEC.md's section skeleton exactly (header block with maturity state and IMG-001 blocker; section 1 registry facts [F]; section 2 rank in suit recomputed from sort_order, not memory; section 3 scoring sourced-or-absent; section 4 iconography baseline at true confidence; section 5 confusion resolvers or 'none required'; section 6 project reading [PS], at most 10 lines, built only from sections 1-5 anchors; section 7 boundaries; section 8 mini claims table, 6-12 rows, claim IDs namespaced SW-NN-C-NN per the spec's claim-namespace rule). Diff every card against drafts/STANDARD_SUIT-COINS-04_Four_of_Coins.md (the working pilot) for structural and disciplinary consistency (line-count band 60-120, same grading legend, same bounded-transcription caveat wording).
- SCORING DISCIPLINE (already settled by the format spec section 3, do not re-litigate or invent a new rule): RULE-1790 pp. 5-6 gives five points to each King and none to the other thirteen cards of a suit; every captured card (including each Swords pip) contributes one card to the captured-card count, never more; no combination (verzicola) value is demonstrated on the record for any suit numeral, and no categorical exclusion is asserted either -- state this as sourced-or-absent per pip, never invent an amount. Swords/Batons are the LONG suits: state their trick order only where a witness transcription for the long-suit order is actually on the corpus; otherwise mark the ordering [UNVERIFIED] rather than assuming an ascending numeral order by default -- do not silently import the round-suit inverse rule, which is sourced only for Coins/Cups.
- No modern cartomancy meanings, no RWS bleed-over (no Ten of Swords ruin scene, no sword-suit-as-conflict imagery, no upright/reversed keywords), no elemental/astrological/Kabbalah/numerology correspondences, no unsourced point amounts or combination values -- identical discipline to every batch this project has run (trump/arie/papi-fool/zodiac/element).
- Work in waves with self-verification before final submission, matching the trump/arie/papi-fool batch pattern: recompute every section-2 rank-in-suit line against the registry sort_order column, check every claims-table row resolves to a source cell or an explicit [UNVERIFIED]/absent marker, grep every drafted file for forbidden terms (pentacles, upright, reversed, Water/Earth-as-suit, RWS scene names), and diff each card against the Four of Coins pilot for format-skeleton and line-count consistency.
- Write a batch verification report at research/pilots/Swords_Pip_Batch_Verification_Report.md with a per-card verdict (PASS / PASS_WITH_CORRECTIONS / FAIL) for all ten cards and a recomputation table showing each card's sort_order, block-start arithmetic, and resulting rank-in-suit.
- OUT OF SCOPE: the four Swords court cards (already handled, see above); any modern cartomancy content; any point value, card count, or combination reach not traceable to a named witness and locator; any other suit's cards.

**Definition of Done:**
- All ten files research/pilots/drafts/STANDARD_SUIT-SWORDS-01_*.md through STANDARD_SUIT-SWORDS-10_*.md exist, each following the SUIT_CARD_FORMAT_SPEC.md skeleton (header/maturity/IMG-001 blocker, sections 1-8 present, claims table with SW-NN-C-NN IDs), each 60-120 lines, none containing a court card id (SUIT-SWORDS-11 through -14) or forbidden terms from the spec's section 5 list.
- research/pilots/Swords_Pip_Batch_Verification_Report.md exists, naming all ten SUIT-SWORDS-01..10 card ids with a verdict and recomputed rank-in-suit for each.
- Every section-2 rank-in-suit arithmetic line is recomputed from the registry's sort_order column (block start 1), not asserted from memory.
- Every section-3 scoring statement is sourced (RULE-1790 pp. 5-6 as transcribed via the Four of Coins pilot's claim chain, or the Justice pilot's verzicola hedge where relevant) or explicitly marked absent/[UNVERIFIED] -- no invented amounts.
...
- ONE CARD ALREADY DONE -- EXCLUDE IT: SUIT-COINS-04 (Four of Coins, sort order 46) already has a complete light-tier study, research/pilots/drafts/STANDARD_SUIT-COINS-04_Four_of_Coins.md, and is the working pilot the format spec itself is built from. Do NOT rewrite, duplicate, or restate it. This task's scope is the OTHER NINE Coins pip cards only: SUIT-COINS-01, 02, 03, 05, 06, 07, 08, 09, 10 (nine cards, not ten -- Coins is the one suit where the count differs from the other three, because its pilot card is already complete).
- COURTS EXCLUDED (confirm, do not re-derive from scratch): SUIT-COINS-11 (Fantina/Maid, the round-suit female-page court, NOT Fante/Jack -- Cups and Coins substitute Fantina for the male Fante used in the long suits Swords/Batons), SUIT-COINS-12 (Cavalier/Cavallo), SUIT-COINS-13 (Queen/Regina), SUIT-COINS-14 (King/Re) are the four Coins court cards, sort orders 53-56, already scoped under the middle court tier (T-MIN-013 pilot 2 of 2; T-MIN-021 middle-tier format, closed DONE). They are OUT OF SCOPE here -- do not write, restate, or touch any Coins court file under this task.
- Write all nine remaining pip cards fresh, one file per card, to research/pilots/drafts/STANDARD_SUIT-COINS-NN_Card_Name.md (NN zero-padded, e.g. STANDARD_SUIT-COINS-01_Ace_of_Coins.md), following research/pilots/SUIT_CARD_FORMAT_SPEC.md's section skeleton exactly (header block with maturity state and IMG-001 blocker; section 1 registry facts [F]; section 2 rank in suit recomputed from sort_order, not memory; section 3 scoring sourced-or-absent; section 4 iconography baseline at true confidence; section 5 confusion resolvers or 'none required'; section 6 project reading [PS], at most 10 lines, built only from sections 1-5 anchors; section 7 boundaries; section 8 mini claims table, 6-12 rows, claim IDs namespaced CO-NN-C-NN per the spec's claim-namespace rule -- note CO-04-C-## is already taken by the existing Four of Coins pilot, so the nine new cards use their own NN, e.g. CO-01-C-01, CO-10-C-07). Diff every new card against the existing drafts/STANDARD_SUIT-COINS-04_Four_of_Coins.md for structural and disciplinary consistency (line-count band 60-120, same grading legend, same bounded-transcription caveat wording) -- this is the same suit, so it is the most direct same-suit consistency check available.
- SCORING DISCIPLINE (already settled by the format spec section 3 and demonstrated in the Four of Coins pilot itself, do not re-litigate or invent a new rule): RULE-1790 pp. 5-6 gives five points to each King and none to the other thirteen cards of a suit; every captured card (including each Coins pip) contributes one card to the captured-card count, never more; no combination (verzicola) value is demonstrated on the record for any suit numeral, and no categorical exclusion is asserted either -- state this as sourced-or-absent per pip, never invent an amount. Coins is a ROUND suit: RULE-1790 cap. II (printed pp. 5-6, as transcribed in the Four of Coins pilot, CL-SC04-008/-009) inverts numeral trick order -- cite the Four of Coins pilot's own transcription directly for the other nine Coins cards rather than re-deriving it, keeping the witness-bounded caveat (secure for the 1790 Florentine witness only).
- No modern cartomancy meanings, no RWS bleed-over (no Ten of Pentacles legacy scene, no 'pentacles' terminology anywhere, no coin-suit-as-wealth imagery, no upright/reversed keywords), no elemental/astrological/Kabbalah/numerology correspondences (no Coins-equals-Earth), no unsourced point amounts or combination values -- identical discipline to every batch this project has run (trump/arie/papi-fool/zodiac/element).
- Work in waves with self-verification before final submission, matching the trump/arie/papi-fool batch pattern: recompute every section-2 rank-in-suit line against the registry sort_order column, check every claims-table row resolves to a source cell or an explicit [UNVERIFIED]/absent marker, grep every drafted file for forbidden terms (pentacles, upright, reversed, Water/Earth-as-suit, RWS scene names), and diff each card against the Four of Coins pilot for format-skeleton and line-count consistency.
- Write a batch verification report at research/pilots/Coins_Pip_Batch_Verification_Report.md with a per-card verdict (PASS / PASS_WITH_CORRECTIONS / FAIL) for all nine new cards (Four of Coins may be listed for reference as already complete / out of scope but is not a required verdict target of this batch) and a recomputation table showing each card's sort_order, block-start arithmetic, and resulting rank-in-suit.
- OUT OF SCOPE: the four Coins court cards (already handled, see above); the existing Four of Coins pilot (already complete, do not rewrite); any modern cartomancy content; any point value, card count, or combination reach not traceable to a named witness and locator; any other suit's cards.

**Definition of Done:**
- All nine files research/pilots/drafts/STANDARD_SUIT-COINS-NN_*.md for NN in 01, 02, 03, 05, 06, 07, 08, 09, 10 exist, each following the SUIT_CARD_FORMAT_SPEC.md skeleton (header/maturity/IMG-001 blocker, sections 1-8 present, claims table with CO-NN-C-NN IDs), each 60-120 lines, none containing a court card id (SUIT-COINS-11 through -14) or forbidden terms from the spec's section 5 list, and none duplicating or rewriting SUIT-COINS-04.
- research/pilots/Coins_Pip_Batch_Verification_Report.md exists, naming all nine SUIT-COINS-01,02,03,05,06,07,08,09,10 card ids with a verdict and recomputed rank-in-suit for each.
- Every section-2 rank-in-suit arithmetic line is recomputed from the registry's sort_order column (block start 43), not asserted from memory.
- Every section-3 scoring statement is sourced (RULE-1790 pp. 5-6 as transcribed via the Four of Coins pilot's claim chain, or the Justice pilot's verzicola hedge where relevant) or explicitly marked absent/[UNVERIFIED] -- no invented amounts.
